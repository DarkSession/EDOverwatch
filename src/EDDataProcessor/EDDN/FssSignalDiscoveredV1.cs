            ...
            foreach (Signals signal in Message.Signals)
            {
                ...
                if (signal.IsStation)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        type = StarSystemFssSignalType.FleetCarrier;
                        {
                            ...
                            {
                                isNew = true;
                                station = new(0, carrierId, 0, 0, 4, 4, 8, StationState.Normal, RescueShipType.No, signal.Timestamp, signal.Timestamp)
                                {
                ...
                else
                {
                    ...
                    if (maelstromMatch.Success)
                    {
                        ...
                        {
                            thargoidMaelstrom = new(0, maelStromName, 20m, 0, Message.Timestamp)
                            {
                                StarSystem = starSystem,
                            };
                            await dbContext.SaveChangesAsync(cancellationToken);
