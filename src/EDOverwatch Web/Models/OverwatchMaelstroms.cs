        ...
        private static async Task<OverwatchMaelstroms> CreateInternal(EdDbContext dbContext, CancellationToken cancellationToken)
        {
            var maelstroms = await dbContext.ThargoidMaelstroms
                ...
                .Select(t => new
                {
                    ...
                    PopulatedAlertsDefended = dbContext.StarSystemThargoidLevels.Count(s => s.Maelstrom == t && s.CycleEnd != null && s.CycleStart!.Start <= s.CycleEnd.Start && s.StarSystem!.OriginalPopulation > 0 && s.State == StarSystemThargoidLevelState.Alert && s.CurrentProgress!.ProgressPercent >= 1m),
                    PopulatedInvasionsDefended = dbContext.StarSystemThargoidLevels.Count(s => s.Maelstrom == t && s.CycleEnd != null && s.CycleStart!.Start <= s.CycleEnd.Start && s.StarSystem!.OriginalPopulation > 0 && s.State == StarSystemThargoidLevelState.Invasion && s.CurrentProgress!.ProgressPercent >= 1m),
                })
